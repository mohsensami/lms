...
function QuizModal({ quizzes, courseId, quizSetId, open, setOpen }) {
  ...
  const submitQuiz = async () => {
    ...
    try {
      const score = await addQuizAssessment(courseId, quizSetId, answers);
      setResult(score);
      router.refresh();
    } catch (error) {
  ...
  const handleClose = () => {
    setOpen(false);
    // Reset local state for next time this modal opens (e.g. a different quiz set).
    setResult(null);
    setQuizIndex(0);
    setAnswers([]);
  };
        ...
        <DialogContent className="sm:max-w-[95%] block">
          ...
          {result ? (
            <div className="flex flex-col items-center gap-4 py-6 text-center">
              ...
              <h3 className="text-xl font-bold text-foreground">آزمون شما ثبت شد</h3>
              <p className="text-muted-foreground">
                شما{' '}
                <span className="font-bold text-foreground">
                  {result.correct} از {result.total}
                </span>{' '}
                سوال را درست پاسخ دادید.
              </p>
              <Badge variant={result.percentage >= 60 ? 'success' : 'destructive'} className="text-sm">
                نمره: {result.percentage}٪
              </Badge>
              <p className="max-w-md text-xs text-muted-foreground">
                این نمره در مدرک پایان دوره‌ی شما (پس از تایید مدرس/ادمین) درج می‌شود.
              </p>
            ...
            <>
              ...
              <div className="py-4">
                ...
                <span className="text-[10px] block text-end">
                  ...
                  A question can have multiple answers & there is no negative
              ...
              <DialogFooter className="flex gap-4 justify-between w-full sm:justify-between">
                ...
                {quizIndex >= lastQuizIndex ? (
                  <Button
                    ...
                    onClick={submitQuiz}
                  >
                    {isSubmitting ? 'در حال ثبت...' : 'Submit'}
                  </Button>
                ) : (
                  <Button
                    className="gap-2 rounded-3xl"
