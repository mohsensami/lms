...
export async function getInstructorDashboardData(dataType) {
  try {
    ...
    // Admins are the site's highest access level: they see every course,
    // enrollment, and review across the whole platform, not just the ones
    // they personally instruct.
    const data =
      user?.role === "admin"
        ? await getAllCoursesDashboardData(true)
